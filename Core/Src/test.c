...
void twist(uint8_t color)
{
	int percent = 7;		// the twist percentage
	int iterations = 30;	// times to repeat
	int A, B;				// line endpoint indexes
	int side;				// loop index over each side
	int cornersX[4] = { 0, SSD1306_WIDTH, SSD1306_WIDTH, 0 };
	int cornersY[4] = { 0, 0, SSD1306_HEIGHT, SSD1306_HEIGHT };
	int index;
	int step;
	...
	for(index = 0;index < iterations; index++)
	{
		...
		{
			SSD1306_DrawLine(cornersX[A], cornersY[A], cornersX[B], cornersY[B], color);
			A = (A + 1) & 3;
			B = (B + 1) & 3;
		}
        ...
        // Move the endpoints inward based on the percent parameter.
        A = 0;
        B = 1;
        for(side=0; side < 4; side++)
        {
			step = cornersX[B] - cornersX[A];
			step = step * percent / 100;
			cornersX[A] += step;

			step = cornersY[B] - cornersY[A];
			step = step * percent / 100;
			cornersY[A] += step;
